document.addEventListener('DOMContentLoaded', () => {
    ...
    function changeLanguage(lang) {
        document.querySelectorAll('[data-i18n]').forEach(element => {
            ...
            if (translations[lang] && translations[lang][key]) {
                ...
                // Якщо це INPUT (для імені та телефону), міняємо Placeholder
                if (element.tagName === 'INPUT') {
                    const cleanText = translation.replace(/[👤📞🛠]/g, '').trim();
                    element.setAttribute('placeholder', cleanText);
                } 
                // Якщо це заголовок чи блок зі специфічною версткою
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', async (e) => {
            ...
            // Беремо текст поточної мови прямо з випадаючого списку
