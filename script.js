document.addEventListener('DOMContentLoaded', () => {
    ...
    /** 5. ЛОГИКА ПОЛЯ ТЕЛЕФОНА (+380 И ТОЛЬКО ЦИФРЫ) **/
    const phoneInput = document.getElementById('formPhone');
    if (phoneInput) {
        // При фокусе, если пусто, ставим +380
        ...
        phoneInput.addEventListener('input', (e) => {
            ...
            // Ограничение длины (Украина: +380 + 9 цифр = 13 символов)
    ...
    /** 6. ВІДПРАВКА В TELEGRAM (2 ОТРИМУВАЧІ + УКР ВІДПОВІДЬ) **/
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', async (e) => {
            ...
            // Валидация: должна быть заполнена вся длина +380XXXXXXXXX (13 символов)
            if (phone.length < 13) {
                alert('Будь ласка, введіть повний номер телефону: +380XXXXXXXXX');
                return;
            }
            ...
            try {
                ...
                if (isAnySuccess) {
                    ...
                    alert('Помилка сервера. Спробуйте ще раз пізніше.');
                }
            } catch (error) {
